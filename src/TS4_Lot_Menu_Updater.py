...
current home zone, extracts that lot's SaveGameLotThumbnail1 (type 0x0F),
center-crops it to a full 16:9 1920x1080 background, encodes it as DXT5,
and patches the background DDS resource in a copy of the supplied menu template.
...
class DBPF:
    def __init__(self, path: Path):
        ...
        if self.index_flags != 0:
            raise ValueError(f"Unsupported compact DBPF index flags 0x{self.index_flags:08X}")
        if self.index_size < 4 + self.entry_count * 32:
...
def get_active_household_and_zone(savegame_data):
    ...
    if isinstance(state, (bytes, bytearray)):
        zone = find_zone_message(bytes(state), household)
    if zone is None:
        zone = find_zone_message(bytes(slot), household)
    if zone is None:
        raise ValueError(f"Could not find a current/home zone for household 0x{household:016X}")
...
def extract_active_lot_thumbnail(save_path):
    ...
    household, zone = get_active_household_and_zone(decoded)
    hi = (zone >> 32) & 0xFFFFFFFF
    lo = zone & 0xFFFFFFFF
    match = next((x for x in pkg.resources(TYPE_SAVEGAME_LOT_THUMBNAIL1) if x.instance_a == hi and x.instance_b == lo), None)
    if match is None:
        raise ValueError(f"Could not find SaveGameLotThumbnail1 for zone 0x{zone:016X}")
...
def make_background(jpg_bytes):
    ...
    # Cover-crop: fill the entire 1920x1080 frame with the actual lot thumbnail.
    # A square source is enlarged to 1920x1920, then cropped vertically to 16:9.
    ...
    # Mild sharpening only; no blur, side panels, or pasted square foreground.
    frame = ImageEnhance.Sharpness(frame).enhance(1.12)
    return frame


def image_to_template_dds(image, header, temp_dir):
    temp = Path(temp_dir) / "_dynamic_menu_temp.dxt5.dds"
...
def compatible_background_entries(pkg):
    ...
    for e in pkg.resources(TYPE_DST_IMAGE):
        if e.file_size != EXPECTED_DDS_SIZE:
            continue
        raw = pkg.raw_resource(e)
        if len(raw) >= 128 and raw[:4] == b"DDS " and raw[84:88] == EXPECTED_FOURCC and struct.unpack_from("<I", raw, 16)[0] == EXPECTED_BG_WIDTH and struct.unpack_from("<I", raw, 12)[0] == EXPECTED_BG_HEIGHT:
            out.append(e)
    return out
...
def main():
    ...
    jpg, household, zone = extract_active_lot_thumbnail(save)
    thumb_instance = ((zone & 0xFFFFFFFF) << 32) | (zone >> 32)
    print(f"Household   : 0x{household:016X}")
    print(f"Zone        : 0x{zone:016X}")
    print(f"Lot thumb   : 0x{thumb_instance:016X}")
    print("Image source: SaveGameLotThumbnail1 (0x0F)")
    ...
    count = patch_template_package(template, output, dds)
